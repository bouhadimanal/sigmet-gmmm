...
// ============================================================
// Cache pour les données de foudre
// ============================================================
let lightningCache = { strikes: [], timestamp: 0 };
const CACHE_TTL = 25000; // 25 secondes
...
// ============================================================
// Récupération des éclairs Blitzortung
// ============================================================
async function fetchLightning() {
  ...
  const endpoints = [
    ...
    'https://map.blitzortung.org/geoserver/data.json',
  ];
  ...
  // Aucun endpoint n'a fonctionné - retourner le cache ou vide
  return lightningCache.strikes || [];
}
...
app.get('/api/lightning', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      source: strikes.length > 0 ? 'blitzortung' : 'no_data',
    });
  } catch (error) {
    res.json({
      success: true,
...
// ============================================================
// API : Radar RainViewer (tuiles météo en temps réel)
// ============================================================
app.get('/api/radar-timestamp', async (req, res) => {
  ...
  try {
    ...
    if (data && data.radar && data.radar.past && data.radar.past.length > 0) {
      ...
      res.json({ success: true, timestamp: latest });
    } else {
...
app.listen(PORT, () => {
  ...
  console.log(`Lightning: http://localhost:${PORT}/api/lightning`);
});
